...
## FoI parameters ##
  # y: number of individuals in each compartment (n.gp*19 matrix)
  # no: number of opposite sexual partners for each compartment (n.gp*19 matrix)
  # uoC: condom use adjustment term for opposite sex (n.gp*19 matrix)
  # ns: number of same sexual partners for each compartment (18*19 matrix)
  # usC: condom use adjustment term for same sex (18*19 matrix)
  # eO: assortative coefficient for heterosexual mixing between ethnic groups 
  #  (vector of 3 for white, black, hispanic)
  ...
  # simgaM: probability of transmission by same sex for 16 HIV+ states
...
FoI=function(y, no, uoC, ns, usC, eO,eS, sigmaFM, sigmaMF, sigmaM, tau, eff.prep, d, s, 
  ...
  ########### mixing matrix mij ##########
  # function to calculate a mixing matrix
  mixMatrix = function(ind, ind.low, ass.e, n, uC){
    xn.total  = xnSum(ass.e, ind, n, uC)
    ...
    xn.j = matrix((1-ass.e) * c(xn.undiag, xn.diag), 6, length(ind)*2, byrow=T) 
    # random mixing
    r.mix = xn.j/xn.total 
    ...
    return(ass.e * k.delta.prop + (1-ass.e) * r.mix)
  }
  
  # mij for female (3 rows; length(m)*2 columns)
  mf = mixMatrix(m,       het.m.l, eO, no, uoC)
  
  # mij for male opposite sex (3 rows; length(f)*2 columns)
  mO = mixMatrix(f,       het.f.l, eO, no, uoC)
  ...
  if (bal==T) {
    ...
    l.loc.m=(rep(m,2) %in% het.m.l) #true if het male low
    np.l=sum(l.loc.m)/3
  ...
  betaI = function(i, ind, mij, sigma, homo=F){
    ...
    if (homo==T) nuC =ns[i,1]*usC[i,1]
    else nuC = no[i,1]*uoC[i,1]
    return(c (nuC * trans.red[i] * sum(beta),     #no PrEP
              nuC * trans.red[i] * sum(betaP)))   #   PrEP
  }
  ...
  gamma = cbind(ds.all * trans.red * gamma.sum,     #no PrEP
                ds.all * trans.red * gamma.sum.p)   #   PrEP
