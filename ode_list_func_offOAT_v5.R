##################################################################################
# An R script to specify the model equations for PWID off OAT.
# 
...
## ODE parameters ##
  # x: a vector containing the number of individuals off OAT
  #   x=c(S1,S2,Sp,Ia,I1,I2,I3,Iap,Ip,Da,D1,D2,D3,T1,T2,T3,O1,O2,O3)
  # x.oat: a vector containing the number of individuals on OAT
...
ode_list_offOAT=function(x,x.oat,beta_o,beta_s,gamma,rho,ws,wp,mu_mat,mo, eta,
...
dS1 = rho*sum(x) - psi*S1 + ws*S2 - lambda*S1   - mu_mat*S1 - mo[1]*S1 - oat.e*S1 + oat.q*x.oat[1] - rho.m*sum(x[4:19])
# the term psi.pr*S2 was removed from dS2 and dSp
dS2 = psi*S1 + wp*Sp      - ws*S2 - lambda*S2   - mu_mat*S2 - mo[2]*S2 - oat.e*S2 + oat.q*x.oat[2] - eta*(S2+S1)
dSp =        - wp*Sp              - lambda.p*Sp - mu_mat*Sp - mo[3]*Sp - oat.e*Sp + oat.q*x.oat[3] + eta*(S2+S1)

dIa = lambda*(S1+S2)-psi*Ia-theta.ai*Ia-mu_mat*Ia-mo[4]*Ia-oat.e*Ia+oat.q*x.oat[4]+rho.m*Ia
...
dO1 = theta.t1O*T1-alpha.re1*O1-theta.1*O1-mu_mat*O1 - mo[17]*O1-oat.e*O1+oat.q*x.oat[17]+rho.m*O1
dO2 = theta.t2O*T2+theta.1*O1-(alpha.re2+theta.2)*O2-mu_mat*O2 - mo[18]*O2-
      oat.e*O2+oat.q*x.oat[18]+rho.m*O2
